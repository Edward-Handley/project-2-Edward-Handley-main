...
def createQueries(in_name, out_name, table):
  ...
  with open(out_name, 'a') as file_out:
    ...
    with open(f'../Raw info/{in_name}.csv') as file_in:
...
in_files = ["member", "Players_test_data_updated", "Teams_test_data", "Trainings_test_data_updated","Venues_test_data_updated"]

# These are the tables that each csv file corresponds to
# NOTE: the order of tables must match the order of the csv files
tables = ["Members", "Playes", "Teams", "Trainings", "Venues"]
